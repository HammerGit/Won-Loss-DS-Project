...
#####   Decision tree #####
dTree <- rpart(WonLost ~ EstRateGroups + OppDurationGroup + EstHoursGroups + Stage4D, data = opptrain, method = "class")
fancyRpartPlot(dTree)

# Run Random Forrest model:
set.seed(300)
rfTree <- randomForest(WonLost ~ EstRateGroups + OppDurationGroup + EstHoursGroups + ServiceOffering +
                       Priority + Stage4D, data=opptrain, importance=TRUE, ntree=2000)
...
lrModel <- glm(WonLost ~ EstRateGroups + OppDurationGroup + EstHoursGroups + ServiceOffering +
                 Priority + Stage4D, data = opptrain, family=binomial(link="logit"))
...
LogReComparo$LogRegPred[LogReComparo$LogRegPredProb >= 0.6] <- 1
